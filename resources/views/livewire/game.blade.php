...
<div
    ...
    @played.stop.window="
           ...
           "
>
    <header class="text-center p-4 border-b-2 border-gray-300 relative">
    ...
    <div class="w-full min-h-[calc(100vh-75px)] flex justify-center items-center @if($status !== GameStatus::ACTIVE) absolute top-0 left-0 @else hidden @endif">
        <div @class([
            ...
            <span class="text-xl flex flex-col justify-center items-center text-center">
                @if($status === GameStatus::LOST)
                    <span class="text-2xl">😭</span> Não conseguiste agarrar esta pista...<p> Abre a próxima caixa para mais pistas! Vais adorar!!!<p>
                @elseif($status === GameStatus::WON)
                <span class="text-2xl ">🎄</span> <span>Parabéns! Podes abrir a próxima caixa para saberes mais sobre a tua prenda </span>
                @endif
